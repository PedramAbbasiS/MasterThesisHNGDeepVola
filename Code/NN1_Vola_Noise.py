...
X_train, X_test, y_train_pure, y_test = train_test_split(
   xx, yy, test_size=0.15, random_state=42)

##noise
def randmult20(y):
    return np.random.uniform(low=y*.99, high=y*1.01)
y_train_noise = np.array([randmult20(y) for y in y_train_pure]) 
X_train_noise, X_val_noise, y_train_noise, y_val_noise = train_test_split(
   X_train, y_train_noise, test_size=0.15, random_state=42)


X_train, X_val, y_train_pure, y_val_pure = train_test_split(
   X_train, y_train_pure, test_size=0.15, random_state=42)


comp = {}
dataset = ['pure','noise']
y_train_sum = [y_train_pure, y_train_noise]
y_val_sum = [y_val_pure, y_val_noise]
for ind in [0,1]:
    y_train = y_train_sum[ind]
    y_val = y_val_sum[ind]
    ...
    NN1.save_weights('NN_HNGarch_weights_'+dataset[ind]+'.h5')
    comp['NN_weights_'+dataset[ind]] = get
    ...
    comp['mean_error_all_'+dataset[ind]] = np.mean(mean_error)
    comp['std_error_'+dataset[ind]] = std_error
    comp['max_error_'+dataset[ind]] = max_error
    comp['max_error_all_'+dataset[ind]] = np.max(max_error)
...
sns.distplot(comp['max_error_noise'], color = 'red', hist = False, label = 'noise')
plt.legend(prop={'size': 16})
plt.title('max errors noise vs. pure')
...
sns.distplot(comp['mean_error_noise'], color = 'red', hist = False, label = 'noise')
plt.legend(prop={'size': 16})
plt.title('mean errors noise vs. pure')
...
sns.distplot(comp['std_error_noise'], color = 'red', hist = False, label = 'noise')
plt.legend(prop={'size': 16})
plt.title('std errors noise vs. pure')
plt.xlabel('std error')
plt.ylabel('Density')
plt.tight_layout()
plt.rcParams.update({'font.size': 16})
plt.savefig('Robustness_Vola_Noise.png', dpi=300)

w_noise = NN1.load_weights('NN_HNGarch_weights_noise.h5')
