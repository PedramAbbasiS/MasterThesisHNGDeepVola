...
which_par = 0 
xx_sen = copy.deepcopy(xx_pure)
xx_sen[:,which_par] = [np.random.normal(np.mean(xx_pure[:,which_par]), np.std(xx_pure[:,which_par]),1)+xx_pure[l,which_par] for l in range(len(xx_pure[:,which_par]))]

yy=data[:,Nparameters+1:]

ind = -1
comp = {}
dataset = ['pure','sensitiv']
for xx in [xx_pure, xx_sen]:
    ind= ind+1
...
sns.distplot(comp['max_error_sensitiv'], color = 'red', hist = False, label = 'sensitive')
plt.legend(prop={'size': 16})
plt.title('max errors pure vs. sensitive')
...
sns.distplot(comp['mean_error_sensitiv'], color = 'red', hist = False, label = 'sensitive')
plt.legend(prop={'size': 16})
plt.title('mean errors pure vs. sensitive')
...
sns.distplot(comp['std_error_sensitiv'], color = 'red', hist = False, label = 'sensitive')
plt.legend(prop={'size': 16})
plt.title('std errors pure vs. sensitive')
