...
%% 
diff = zeros(Nmaturities,Nstrikes,size(data_vola_clear,1));
pricer = zeros(Nmaturities,Nstrikes,size(data_vola_clear,1));
for t = 1:Nmaturities
    for k =1:Nstrikes
        for i=1:size(data_vola_clear,1)
            pricer(t,k,i)=blsprice(1,K(k),r,Maturity(t)/252,sqrt(252*data_vola_clear(i,5)));%(w+a)/(1-a*g^2-b))); 
...
figure, for i=1:84, surf(diff(:,:,i));hold on,set(gca,'ZScale','log','ColorScale','log'),end
