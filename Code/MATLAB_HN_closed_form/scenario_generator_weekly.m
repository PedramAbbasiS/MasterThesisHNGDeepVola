...
save(strcat('data_price_week_',num2str(num_week),num2str(length(data_price)),'_',num2str(r),'_',num2str(min(K)),'_',num2str(max(K)),'_',num2str(min(Maturity)),'_',num2str(max(Maturity))),'data_price')
% Summary
    fprintf('\n')
    disp(strcat("rate/number of pos. price-data: ",num2str((sum(sum((scenario_data(:,8:end)>=0))))/(Nsim*Nstrikes*Nmaturities)*100),"% , ",num2str((sum(sum((scenario_data(:,8:end)>=0)))))));
