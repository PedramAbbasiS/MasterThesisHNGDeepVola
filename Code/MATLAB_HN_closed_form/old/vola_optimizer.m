...
Maturity = round(30:30:240,0);
%Maturity =60;
bound =[100,100];
method = 'linear';
surface = surface_generator(year,stock_ind,Strikes,Maturity,path,bound,0,0,0,1,method);
figure,surf(surface);
%surface = reshape(rBergomiTrainSetnew(1,5:end),[11,8]);
...
vola0 =imp_vola_mat(prices0,r,Strikes,S,Maturity);
%%
clc;
...
%gs = GlobalSearch;
%[x,f] = run(gs,problem)
args=fmincon(@(x) min_vola(x(1),Strikes,S,Maturity,r,x(2),x(3),x(4),x(5),surface),x0,[],[],[],[],lb,ub,@(x) nonlincon(x),options);
prices_opt =hn_matrix(args(1),Strikes,S,Maturity,r,args(2),args(3),args(4),args(5));
vola_opt = imp_vola_mat(prices_opt,r,Strikes,S,Maturity);
...
vola_opt = imp_vola_mat(prices_opt,r,Strikes,S,Maturity);
figure, surf(X,Y,surface);hold on, surf(X,Y,vola_opt);legend('Surface','Opt');
